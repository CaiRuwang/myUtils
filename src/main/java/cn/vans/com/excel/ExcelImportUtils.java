...
import cn.vans.com.logger.LogHelper;
...
public class ExcelImportUtils {
    ...
    public void  readRowSelectValue(){
        System.out.println("=====");
    ...
    public  <T> List<T> getExcelData2(String filePath, Class<T> clazz, int startRowNum) throws Exception {
        ...
        try {
            ...
            if (rowNum < stratNum)
                throw new RuntimeException("Excel中数据请从第" + startRowNum + "行开始！");
            for (int i = stratNum; i <= rowNum; i++) {
                T t = clazz.newInstance();
                Row row = sheet.getRow(i);
                if(row.getZeroHeight()) continue;
               /// System.out.println(row.getZeroHeight()  +" ;" + getCellVal(row.getCell(0))+";" +getCellVal(row.getCell(1))+";"+getCellVal(row.getCell(2))+";"+getCellVal(row.getCell(3))+";"+getCellVal(row.getCell(4))+";"+getCellVal(row.getCell(5)));
     ...
     * </ul>
     */
    public <T> List<T> getExcelData(String filePath, Class<T> clazz, int startRowNum) throws Exception {
        List<T> dateList = null;
        try {
            dateList = new ArrayList<T>();
            getWorkBook(filePath);
    ...
    private void getWorkBook(String filePath) throws Exception {
        File file = null;
        try {
        ...
        if (!file.exists())
            throw new RuntimeException("导入文件不存在,请核对！");
